  ...
  runSrtn(filesArray: FileCH[]): [FileCH[]] {
    ...
    for (let file = 0; file < filesArray.length; file++) {
      ...
      if (filesArray.length > 1 && file < filesArray.length - 1) {
        filesArray[file].endingRr = filesArray[file + 1].arrivalTime - 1;
      } else {
        filesArray[file].endingRr = filesArray[file].codeLines.length - 1;
      }
    // hacer una variable que sepa cual es el menor y si ya terminaron los arrival times termine con esta
      for (let instruccion = filesArray[file].arrivalTime; instruccion <= filesArray[file].endingRr; instruccion++) {
      ...
      filesArray[file].burstCPU -= filesArray[file].endingRr - filesArray[file].arrivalTime;
      ...
      filesArray[file].arrivalTime = filesArray[file].endingRr;
      
      this.filesStateList[file] = [filesArray[file], acumulador];

      if (filesArray[file].burstCPU > 0) {
        file = -1;
      } 
